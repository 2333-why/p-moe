"""Stage 3 PEFT LoRA continued pretraining entrypoint.

This script is safe at import time. Model loading and training only happen from
main(), after CLI arguments are parsed by the user.
"""
...
from src.lora_utils import apply_lora, prepare_model_for_lora_training, print_trainable_parameters, router_lora_todo
...
def build_parser() -> argparse.ArgumentParser:
    """Build the CLI parser for LoRA continued pretraining."""
    parser = argparse.ArgumentParser(description="LoRA continued pretraining scaffold for p-MoE Stage 3.")
    ...
    parser.add_argument("--torch_dtype", default=None, choices=["auto", "bf16", "fp16", "fp32"])
    parser.add_argument("--bf16", type=str_to_bool, nargs="?", const=True, default=None)
    parser.add_argument("--fp16", type=str_to_bool, nargs="?", const=True, default=None)
...
def normalize_config(args: argparse.Namespace) -> Dict[str, Any]:
    """Merge YAML and CLI configuration and fill safe defaults."""
...
def run_training(config: Dict[str, Any]) -> None:
    """Load model/data, attach LoRA adapters, and run Trainer."""
    ...
    model = AutoModelForCausalLM.from_pretrained(
        config["model_name"],
        local_files_only=bool(config["local_files_only"]),
        torch_dtype=resolve_torch_dtype(str(config["torch_dtype"])),
        trust_remote_code=bool(config.get("trust_remote_code", True)),
    )
...
def main() -> None:
    """Parse arguments and launch LoRA training."""
    parser = build_parser()
